...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler("movilesmanager_documental.log"),  # Logs guardados en un archivo
        logging.StreamHandler(),  # Logs también en consola
    ]
)

class MovilesManagerDocumental:
    def __init__(self, uri, database_name, collection_name):
        """Inicializa el componente MovilesManagerDocumental."""
        self.uri = uri
    ...
    def crear_documento(self, documento):
        """Insertar un nuevo documento en la colección de móviles"""
        try:
    ...
    def leer_documentos(self, filtro={}):
        """Leer documentos de la colección de móviles según un filtro"""
        try:
    ...
    def actualizar_documento(self, filtro, actualizacion):
        """Actualizar un documento en la colección de móviles"""
        try:
    ...
    def eliminar_documento(self, filtro):
        """Eliminar un documento de la colección de móviles"""
        try:
...
# Ejemplo de uso del componente MovilesManagerDocumental
if __name__ == "__main__":
    # Configurar el componente
    db_manager = MovilesManagerDocumental(
        uri="mongodb://localhost:27017",
        database_name="1dam",
        collection_name="moviles"
    )
    ...
    try:
        ...
        db_manager.crear_documento({"marca": "Samsung", "modelo": "Galaxy S22", "color": "Negro", "almacenamiento": "128GB"})
        db_manager.crear_documento({"marca": "Apple", "modelo": "iPhone 13", "color": "Blanco", "almacenamiento": "256GB"})
        ...
        db_manager.iniciar_transaccion()
        db_manager.actualizar_documento({"modelo": "Galaxy S22"}, {"almacenamiento": "256GB"})
        db_manager.confirmar_transaccion()

        # Eliminar un documento
        db_manager.iniciar_transaccion()
        db_manager.eliminar_documento({"modelo": "iPhone 13"})
        db_manager.confirmar_transaccion()
